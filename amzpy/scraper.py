...
from amzpy.parser import parse_pagination_url, parse_product_page, parse_search_page
from amzpy.session import DEFAULT_CONFIG, AmzSession
...
class AmazonScraper:
    ...
    def __init__(
        ...
        self.session = AmzSession(
            country_code=country_code, impersonate=impersonate, proxies=proxies
        )
    ...
    def get_product_details(self, url: str) -> Optional[Dict]:
        ...
        product_data = parse_product_page(
            html_content=response.text, url=product_url, country_code=self.country_code
        )
        ...
        print(
            f"Successfully extracted data for: {product_data.get('title', 'Unknown Product')[:50]}..."
        )
        return product_data

    def search_products(
        self, query: str = None, search_url: str = None, max_pages: int = 1
    ) -> List[Dict]:
        """
        ...
        if not search_url and query:
            search_url = (
                f"https://www.amazon.{self.country_code}/s?k={query.replace(' ', '+')}"
            )
        ...
        while current_url and current_page <= max_pages:
            ...
            products = parse_search_page(
                response.text, base_url, country_code=self.country_code
            )
