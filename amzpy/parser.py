...
def parse_product_page(
    html_content: str, url: str = None, country_code: str = None
) -> Optional[Dict]:
    """
    ...
    if (
        ...
        or "api-services-support@amazon.com" in html_content
    ):
    ...
    try:
        ...
        rating = None
        rating_element = soup.select_one("#acrPopover") or soup.select_one(
            "span.a-icon-alt"
        )
        if rating_element:
...
def parse_search_page(
    html_content: str, base_url: str = None, country_code: str = None
) -> List[Dict]:
    """
            ...
            try:
                ...
                if title_link:
                    ...
                    if href:
                        ...
                        product_data["url"] = format_canonical_url(
                            product_url, asin, country_code
                        )
                                ...
                                if price_fraction:
                                    ...
                                    if fraction_text and fraction_text != ".":
                                        product_data["price"] = float(
                                            f"{price_text}.{fraction_text}"
                                        )
                                else:
                                    product_data["price"] = float(price_text)
                            except ValueError:
                                print(
                                    f"Warning: Could not convert price parts: '{price_text}' and '{fraction_text if price_fraction else ''}'"
                                )

                # Extract currency symbol if not already found
                if "currency" not in product_data and container.select_one(
                    ".a-price-symbol"
                ):
                    product_data["currency"] = container.select_one(
                        ".a-price-symbol"
                    ).text.strip()
                    ...
                    if price_match:
                        ...
                        if price_str and price_str != ".":
                            try:
                                ...
                                print(
                                    f"Warning: Could not convert original price string: '{price_str}'"
                                )
                ...
                for selector in rating_selectors:
                    ...
                    if rating_element:
                        ...
                        if rating_element.get(
                            "aria-label"
                        ) and "out of 5" in rating_element.get("aria-label"):
                            rating_text = rating_element.get("aria-label")
                        # Try alt text next
                        elif rating_element.get(
                            "alt"
                        ) and "out of 5" in rating_element.get("alt"):
                        ...
                        rating_match = re.search(
                            r"([\d\.]+)(?:\s+out\s+of\s+5)?", rating_text
                        )
                        if rating_match:
                            ...
                            if rating_str and rating_str != ".":
                                try:
                                    ...
                                    print(
                                        f"Warning: Could not convert rating string: '{rating_str}'"
                                    )
                            break
                        ...
                        if reviews_match:
                            ...
                            if count_text and count_text != ".":
                                try:
                                    ...
                                    print(
                                        f"Warning: Could not convert reviews count: '{count_text}'"
                                    )
                            break
                ...
                product_data["prime"] = any(
                    container.select_one(selector) for selector in prime_selectors
                )
                            ...
                            if color_name:
                                ...
                                canonical_color_url = (
                                    format_canonical_url(
                                        color_url, color_asin, country_code
                                    )
                                    if color_asin
                                    else color_url
                                )
                ...
                badge_text = None
                badge_element = container.select_one(
                    ".a-badge-text"
                ) or container.select_one('[aria-label*="Choice"]')
                if badge_element:
                ...
                delivery_element = container.select_one(
                    '.a-row:-soup-contains("delivery")'
                ) or container.select_one('[aria-label*="delivery"]')
                if delivery_element:
                    delivery_text = delivery_element.text.strip()
                    product_data["delivery_info"] = delivery_text

                # Extract "Deal" information
                deal_element = container.select_one(
                    'span:-soup-contains("Deal")'
                ) or container.select_one('.a-badge:-soup-contains("Deal")')
                if deal_element:
...
def parse_pagination_url(html_content: str, base_url: str = None) -> Optional[str]:
    ...
    next_link = (
        soup.select_one("a.s-pagination-next:not(.s-pagination-disabled)")
        or soup.select_one("li.a-last:not(.a-disabled) a")
        or soup.select_one('a:has(span:contains("Next"))')
        or soup.select_one('a[aria-label="Go to next page"]')
    )
